...
import com.jsprite.web.commons.LuceneData;
import com.jsprite.web.commons.Novel;
...
public class LuceneUtil {
	...
	public static synchronized boolean createIndex(LuceneData data) {
		...
		Directory d = null;
		try {
			Path path = FileSystems.getDefault().getPath(Version.LATEST.toString(), indexPath);
			d = FSDirectory.open(path);
			...
			// System.out.println("索引结束,共有索引{}个" +
			// indexWriter.numDocs()+":"+doc.get("id")+":"+doc.get("author"));
	 ...
	 * @return
	 */
	public static ArrayList<LuceneData> search(String[] fileds, Occur[] occurs,
	 ...
	 */
	public static ArrayList<LuceneData> search(String[] fileds, Occur[] occurs,
			String keyWord, String bigtype, String subType, Integer page,
			Integer pageSize) {
		try {
			// ---------初始化---------------------------------------------------
			Path path = FileSystems.getDefault().getPath(Version.LATEST.toString(), indexPath);
			...
			booleanquery.add(mq, Occur.MUST);
			...
			// 过滤数字区间
			// NumericRangeQuery<Integer> spanquery =
			// NumericRangeQuery.newIntRange("id", begin, end, true, true);
			// booleanquery.add(spanquery, Occur.MUST);

			// 过滤时间区间(时间的getTime比大小)
			// NumericRangeQuery<Integer> spanquery =
			// NumericRangeQuery.newLongRange("id", begin, end, true, true);
			// booleanquery.add(spanquery, Occur.MUST);
			...
			// -------------排序--------------------------------------------------------
			/*
			 * 多字段排序，设置在前面的会优先排序 //true:降序 false:升序 SortField[] sortFields = new
			 * SortField[3]; SortField top = new SortField("isTop",
			 * SortField.INT, true); SortField hits = new
			 * SortField("contentHits", SortField.INT,true); SortField pubtime =
			 * new SortField("publishTime",SortField.LONG, true); sortFields[0]
			 * = top; sortFields[1] = hits; sortFields[2] = pubtime; Sort sort =
			 * new Sort(sortFields);
			 */
			...
			for (ScoreDoc scdoc : docs) {
				...
				tokenStream = TokenSources.getAnyTokenStream(
						searcher.getIndexReader(), scdoc.doc, "name",
				...
				tokenStream = TokenSources.getAnyTokenStream(
						searcher.getIndexReader(), scdoc.doc, "author",
						AnalyzerUtil.getIkAnalyzer());
				...
				tokenStream = TokenSources.getAnyTokenStream(
						searcher.getIndexReader(), scdoc.doc, "outline",
						AnalyzerUtil.getIkAnalyzer());
				...
				tokenStream = TokenSources.getAnyTokenStream(
						searcher.getIndexReader(), scdoc.doc, "type",
						AnalyzerUtil.getIkAnalyzer());
	...
	private static Document getDocument(LuceneData data) {
		...
		doc.add(new TextField("id", data.getId(), Store.YES));
		doc.add(new TextField("name", data.getName(), Store.YES));
		doc.add(new TextField("author", data.getAuthor(), Store.YES));
		doc.add(new TextField("outline", data.getOutline(), Store.YES));
		doc.add(new TextField("type", data.getType(), Store.YES));
		doc.add(new TextField("updateTime", data.getUpdateTime(), Store.YES));
		doc.add(new TextField("imgPath", data.getImgPath(), Store.YES));
		doc.add(new TextField("imgUrlPath", data.getImgUrlPath() == null ? ""
				: data.getImgUrlPath(), Store.YES));
		doc.add(new TextField("content", data.getContent() == null ? "" : data
				.getContent(), Store.YES));
		doc.add(new TextField("link_url", data.getLink_url(), Store.YES));
		doc.add(new TextField("hot", Long.toString(data.getHot()), Store.YES));
		doc.add(new TextField("clickPoint", Long.toString(data.getClickPoint()),Store.YES));
		doc.add(new TextField("bigtype", data.getBigtype(), Store.YES));
		doc.add(new TextField("typeid", data.getTypeid(), Store.YES));
		return doc;
	}
	...
	public static void main(String[] args) {
		...
		// String[]{"classpath:com/springResource/*.xml"});
		// NovelService novelService =
		// (NovelService)springContext.getBean("novelService");
		// System.out.println("novelService"+novelService);
		//
		// ArrayList<Novel> list=novelService.getNovelList(21, 100);
		// ArrayList<LuceneData>
		// transforlist=LuceneService.transformation(list);
		...
		Occur[] occurs = new Occur[] { Occur.SHOULD, Occur.SHOULD,
				Occur.SHOULD, Occur.SHOULD };
		ArrayList<LuceneData> list = LuceneUtil.search(fileds, occurs, "初雪", 1,
				10);
